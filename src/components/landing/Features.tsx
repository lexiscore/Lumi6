...
export default function Features() {
  return (
    <div id="features" className="bg-neutral-50 py-24 sm:py-32 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 relative">
        <div className="mx-auto max-w-3xl lg:text-center">
          <div className="inline-flex items-center rounded-full bg-white/80 backdrop-blur-sm border border-neutral-200 px-6 py-3 text-sm font-medium text-neutral-700 mb-8 shadow-subtle">
            Comprehensive Assessment Platform
          </div>
          <h2 className="text-4xl sm:text-5xl font-bold tracking-tight leading-tight text-neutral-900 mb-6 font-display">
            Everything you need for
            <span className="block bg-gradient-purple bg-clip-text text-transparent">workforce evaluation</span>
          </h2>
          <p className="text-xl leading-8 text-neutral-600 max-w-2xl mx-auto font-normal">
        ...
        <div className="mx-auto mt-20 max-w-6xl">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3 md:grid-cols-2">
            {features.map((feature, index) => (
              <div 
                key={feature.name} 
                className="group relative bg-white/90 backdrop-blur-sm rounded-2xl border border-neutral-200 p-8 hover:border-neutral-300 hover:shadow-medium transition-all duration-300 hover:scale-[1.02]"
              >
                <div className="relative">
                  <div className={`inline-flex h-14 w-14 items-center justify-center rounded-2xl ${feature.bgColor} border ${feature.borderColor} ${feature.color} group-hover:shadow-lg transition-all duration-300`}>
                    <feature.icon className="h-7 w-7" aria-hidden="true" />
                  </div>
                  <h3 className="mt-6 text-xl font-semibold text-neutral-900 font-display">
                    {feature.name}
                  </h3>
                  <p className="mt-4 text-base leading-7 text-neutral-600 font-normal">{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
